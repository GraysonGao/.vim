...
" 6 -> steady bar (xterm).
let &t_SI = "\e[5 q" " SI is INSERT mode
let &t_EI = "\e[2 q" " EI is NORMAL mode
...
" 按 U 执行 redo
noremap U <c-r>
" shift 回车, 在下一行输入（在一些 Linux 终端中无效，对某些终端来说 s-cr 和 cr 一样）
...
vnoremap <leader>x "+x
" 插入模式下按 <leader>w 保存文件。<silent> 表示静默执行，不需要在底部输出执行信息。
inoremap <silent> <leader>w <Esc>:w<cr>a
" 按 <leader>nv 编辑 vimrc
if (g:isWindows)
    nnoremap <leader>nv :tabe $vim/_vimrc<cr>
elseif (g:isLinux)
    nnoremap <leader>nv :tabe ~/.vim/vimrc<cr>
endif
...
" grvubox 主题（亮/暗），solarized 增强版。
Plug 'morhetz/gruvbox'
" PaperColor 主题（亮/暗）
Plug 'NLKNguyen/papercolor-theme'
" onehalf 主题（亮/暗），PaperColor 增强版。
Plug 'sonph/onehalf'
" The Nerd Tree, 目录树
